...
def execute(upstream_output, **kwargs):
    ...
    is_better = upstream_output.get('is_better', True)
    is_active = is_better
    ...
    model_record = {
        ...
        'hyperparameters': json.dumps({
            ...
            'random_state': 42,
            'n_init': 50,
            'best_run_id': upstream_output.get('best_run_id'),
        }),
        'training_metrics': json.dumps({
            'silhouette_score': upstream_output['silhouette_score'],
            'previous_silhouette_score': upstream_output.get('previous_silhouette_score'),
            'inertia': upstream_output['inertia'],
        }),
        'status': 'production' if is_active else 'staged',
    ...
    cursor.execute(
        "UPDATE ml_models SET is_active = false WHERE model_name = 'kmeans_rfm_segmentation'"
    )
